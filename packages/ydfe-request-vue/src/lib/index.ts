...
const vueInstance: ReqVueConstructor = new Vue();
 ...
 */
function requestFailureHandler(error: RequestError) {
    ...
    // 处理弹窗信息，全局配置弹窗参数
    vueInstance.$Notice && vueInstance.$Notice.config({
        top: 60,
        duration: 0
    });

    if (error.response) {
        vueInstance.$Notice && vueInstance.$Notice.error({
 ...
 * @return {Object} 返回信息
 */
function requestSuccessHandler(response: AxiosResponse) {
    try {
...
function handlerGlobalLoadingFormRequest(config) {
    ...
    // TODO
        ...
        if (inRequestNum === 0) {
            hasLoadingShow = true;
            vueInstance.$Spin && vueInstance.$Spin.show({
                render: h => {
                    return h('div', [
                        h('Icon', {
...
function handlerGlobalLoadingFormResponse() {
    ...
    if (typeof request.hooks.hideGlobalLoading === 'function') {
        ...
        vueInstance.$Spin && vueInstance.$Spin.hide();
...
const axiosDefaultConf = {
    ...
    adapter(config: RequestAxiosRequestConfig) {
        // 处理加载提示逻辑
    ...
    transformResponse: [
        (data: AxiosRequestConfig['transformRequest']) => {
...
interface ReqVueConstructor extends Vue {
    $Notice: {
        config(props: {
            top: number,
            duration: Number
        } & Key): void;
        error(props: { title: string, desc: String, duration?: number } & Key): void;
    },
    $Spin: {
        show: (props: {
...
export interface RequestError<T = any> extends AxiosError {
    isSilent?: boolean
}
export interface RequestAxiosRequestConfig<T = any> extends AxiosRequestConfig {
    pathArgs?: Key;
}

export interface HooksProps {
    handlerRestfulUrl?: (url: string, params: Key) => void,
    beforeFailure?: (error: RequestError) => RequestError,
    showGlobalLoading?: (config: RequestAxiosRequestConfig) => void;
    hideGlobalLoading?: (config?: RequestAxiosRequestConfig) => void;
    beforeAdapter?: (config?: RequestAxiosRequestConfig) => Object;
    beforeResponse?: (data: AxiosRequestConfig['transformRequest']) => AxiosRequestConfig['transformRequest'];
    beforeRequest?: (config: AxiosRequestConfig) => AxiosRequestConfig,
    beforeSuccess?: (response: AxiosResponse) => AxiosResponse;
}
...
export interface RequestProps {
    hooks: HooksProps,
    install?: (Vue: ReqVueConstructor, opt: any) => void,
}
